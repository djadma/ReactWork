...
const Mutations = {
  ...
  async removeFromCart(parent, args, ctx, info) {
    ...
    //3. delete the card item
    return ctx.db.mutation.deleteCartItem({
      where: {id: args.id},
    }, info)
  },
  async createOrder(parent, args, ctx, info) {
    //1. query the current user and make sure sign in too
    const { userId } = ctx.request;
    if(!userId) {
    ...
    //2. recalculate the total fir the price
    ...
    //3. create the strip charge (turn token into money)
    ...
    //6. clean up - clear the users cart, delete car items
    const cartItemIds = user.cart.map(cartItem => cartItem.id);
    await ctx.db.mutation.deleteManyCartItems({
      where: { id_in: cartItemIds,}
    })
    //7. return the order to the clint
    return order;
  }
};
